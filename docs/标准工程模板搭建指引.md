...
#### 安装
##### 方案一：自动安装
`eslint --init` 根据项目需要生成

##### 方案二：手动安装
方案一：
```
    npm i -D eslint babel-eslint eslint-config-airbnb eslint-plugin-import eslint-plugin-jsx-a11y eslint-plugin-react eslint-formatter-pretty @typescript-eslint/parser @typescript-eslint/eslint-plugin 
```
